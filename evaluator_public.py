...
def eva_data(df_map):
    ...
    for item in df_map.iterrows():
        ...
        if set_name in ["CLEF", "JPO", "UOB", "USPTO"]:
            ...
            img_path = os.path.join(base_dir,"Data","public_data",set_name,"{}.png".format(item["Key"]))
        ...
        if not os.path.exists(csv_res_path):
            df_row.to_csv(csv_res_path, header=["key","Img_path","GD","OCMR","Group","Is_OCMR_true"], index=False, mode='a')
        else:
...
if __name__ == "__main__":
    ...
    # 如果不存在结果，则对数据进行分析
    if not os.path.exists(csv_res_path):
        smiles_map = os.path.join(base_dir,"Data","public_data","refence_smiles.csv")
        df_map = pd.read_csv(smiles_map)
        df_map = df_map[df_map["Group"].isin(["CLEF", "JPO", "UOB", "USPTO"])]
        df_list = [df_map[:3000],df_map[3000:6000],df_map[6000:9000],df_map[9000:]]
        po = Pool(4)
    ...
    writer.save()
    print("分组报告保存在:{}".format(os.path.join(base_dir, "all_result_public.xlsx")))